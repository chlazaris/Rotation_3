...
\labday{March 25 2014}
\experiment{Write and run script that calculates correlations for Hi-C contact matrices after removing lines and columns that sum to zero.}
...
\labday{March 26th 2014}
\experiment{Create a matrix with all Lieberman-Aiden data}
...
\labday{Tuesday 8 April 2014}
\experiment{Plan for the next month}

Tasks of the day:

\begin{enumerate}
	\item Download and install genlasso. Try to understand 
	the corresponding documentation.
	\item In the next month you must complete the introduction
	for the paper that we will submit to NAR. 
\end{enumerate}

% \begin{lstlisting}[language=bash]
% \end{lstlisting}

Genlasso downloaded and installed. ~\emph{Caution:} It is possible
to install R packages to R on the cluster. A local personal library
is automatically created in your local directory and all the packages 
are installed there (see R directory in home directory)! 

\vskip 0.5cm

\textbf{NAR Introduction}\\

I am supposed to write an extensive review (about ten pages) which
will consist of: 

\begin{itemize}
	\item 30\% Biological question and background information.
	\item 60\% The current methods and their main findings.
    \item 10\% Strengths of this study, unanswered questions and my own ideas.
\end{itemize}
